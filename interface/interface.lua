	...
	for _, element in ipairs(table.config) do
		...
		elseif element.type == 'checkbox' then
			...
			tmp:SetEventListener('OnValueChanged', function(this, event, checked)
				NetherMachine.config.write(table.key .. '_' .. element.key, checked)
			end)

			tmp:SetChecked(NetherMachine.config.read(table.key .. '_' .. element.key, element.default or false))
		...
		elseif element.type == 'spinner' then
			...
			tmp_spin:SetNumber(
				NetherMachine.config.read(table.key .. '_' .. element.key, element.default)
			)
			...
			tmp_spin:SetEventListener('OnValueChanged', function(this, event, userInput, number)
				...
				NetherMachine.config.write(table.key .. '_' .. element.key, number)
			end)
		...
		elseif element.type == 'checkspin' then
			...
			tmp_spin:SetNumber(
				NetherMachine.config.read(table.key .. '_' .. element.key .. '_spin', element.default_spin or 0)
			)
			tmp_spin:AddStyleSheet(spinnerStyleSheet)
			tmp_spin:ApplySettings()
			...
			tmp_spin:SetEventListener('OnValueChanged', function(this, event, userInput, number)
				if not userInput then return end
				NetherMachine.config.write(table.key .. '_' .. element.key .. '_spin', number)
			end)
			...
			tmp_check:SetEventListener('OnValueChanged', function(this, event, checked)
				NetherMachine.config.write(table.key .. '_' .. element.key .. '_check', checked)
			end)

			tmp_check:SetChecked(NetherMachine.config.read(table.key .. '_' .. element.key .. '_check', element.default_check or false))
		...
		elseif element.type == 'combo' or element.type == 'dropdown' then
			...
			tmp_list:SetEventListener('OnValueChanged', function(this, event, value)
				NetherMachine.config.write(table.key .. '_' .. element.key, value)
			end)

			tmp_list:SetValue(NetherMachine.config.read(table.key .. '_' .. element.key, element.default))
		...
		elseif element.type == "input" then
			...
			tmp_input:SetText(NetherMachine.config.read(table.key .. '_' .. element.key, element.default or ''))

			tmp_input:SetEventListener('OnEditFocusLost', function(this)
				NetherMachine.config.write(table.key .. '_' .. element.key, this:GetText())
			end)
...
function NetherMachine.interface.fetchKey(keyA, keyB, default)
	local selectedProfile = NetherMachine.config.read(keyA .. '_profile', 'Default Profile')
	if selectedProfile then
		return NetherMachine.config.read(keyA .. selectedProfile .. '_' .. keyB, default)
	else
		return NetherMachine.config.read(keyA .. '_' .. keyB, default)
	end
end

function NetherMachine.interface.writeKey(keyA, keyB, value)
	local selectedProfile = NetherMachine.config.read(keyA .. '_profile', 'Default Profile')
	if selectedProfile then
		return NetherMachine.config.write(keyA .. selectedProfile .. '_' .. keyB, value)
	else
		return NetherMachine.config.write(keyA .. '_' .. keyB, value)
	end
end

function NetherMachine.interface.buildGUI(config)
	...
	if config.profiles == true and config.key_orig then
		...
		createButton:SetEventListener('OnClick', function()
			...
			profileButton:SetEventListener('OnClick', function()
				...
				local profiles = NetherMachine.config.read(config.key_orig .. '_profiles', {{key='default',text='Default'}})
				...
				if profileName ~= '' then
					...
					NetherMachine.config.write(config.key_orig .. '_profiles', profiles)
					NetherMachine.config.write(config.key_orig .. '_profile', pkey)
					newWindow:Hide()
					parent:Hide()
					parent:Release()
					NetherMachine.interface.buildGUI(config)
				end
		...
		deleteButton:SetEventListener('OnClick', function()
			local selectedProfile = NetherMachine.config.read(config.key_orig .. '_profile', 'Default Profile')
			local profiles = NetherMachine.config.read(config.key_orig .. '_profiles', {{key='default',text='Default'}})
			if selectedProfile ~= 'default' then
				for i,p in ipairs(profiles) do
					if p.key == selectedProfile then
						profiles[i] = nil
						NetherMachine.config.write(config.key_orig .. '_profiles', profiles)
						NetherMachine.config.write(config.key_orig .. '_profile', 'default')
						parent:Hide()
						parent:Release()
						NetherMachine.interface.buildGUI(config)
					end
		...
		local profiles = NetherMachine.config.read(config.key_orig .. '_profiles', {{key='default',text='Default'}})
		local selectedProfile = NetherMachine.config.read(config.key_orig .. '_profile', 'default')
		...
		profile_dropdown:SetEventListener('OnValueChanged', function(this, event, value)
			if selectedProfile ~= value then
				NetherMachine.config.write(config.key_orig .. '_profile', value)
				parent:Hide()
				parent:Release()
				NetherMachine.interface.buildGUI(config)
			end
		...
		profile_dropdown:SetValue(NetherMachine.config.read(config.key_orig .. '_profile', 'Default Profile'))
		...
		NetherMachine.config.write(config.key_orig .. '_profile', false)
	end


	if config.key_orig then
		parent:SetEventListener('OnDragStop', function(self, event, left, top)
			NetherMachine.config.write(config.key_orig .. '_window', {left, top})
		end)
		local left, top = unpack(NetherMachine.config.read(config.key_orig .. '_window', {false, false}))
...
NetherMachine.interface.init = function()
	NetherMachine.interface.minimap.create()
end
...
local coreNMconfig = {
    ...
    resize = false,
	config = {
        { type = "header", justify = 'LEFT', text = NetherMachine.addonName },
        { type = "spacer", size = 0, pull = 16 },
		{ type = "header", justify = 'RIGHT', text = "v" .. NetherMachine.version },
		{ type = "rule" },
...
NetherMachine.interface.coreNMconfigFrame = NetherMachine.interface.buildGUI(coreNMconfig)
NetherMachine.interface.coreNMconfigFrame.parent:Hide()

NetherMachine.interface.coreNMconfigFrame.parent:SetEventListener('OnClose', function(self)
	NetherMachine.interface.coreNMconfigFrame.parent:Hide()
end)

NetherMachine.interface.showNetherMachineConfig = function()
	if NetherMachine.interface.coreNMconfigFrame.parent:IsShown() then
		NetherMachine.interface.coreNMconfigFrame.parent:Hide()
	else
		NetherMachine.interface.coreNMconfigFrame.parent:Show()
	end
end
