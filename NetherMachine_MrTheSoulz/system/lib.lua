...
NetherMachine.library.register('mtsLib', {
		...
		if (GetTime() - _queueTime) > 10 then
			...
			if _queueSpell then
				if _queueSpell == spellId then
					if NetherMachine.parser.lastCast == GetSpellName(spellId) then
						_queueSpell = nil
						_queueTime = 0
					end
				return true
					...
					if not mts.immuneEvents(mts.unitCache[i].key) then
						if UnitThreatSituation(mts.unitCache[i].key) and UnitThreatSituation(mts.unitCache[i].key) >= 2 then
							if mts.Infront(mts.unitCache[i].key) then
								NetherMachine.dsl.parsedTarget = mts.unitCache[i].key
								return true 
							end
						end
					end
	...
	** Priest - Prayer of Healing **
	...
	We cache this count because NM cicles (therefore calling it) too fast and we dont 
	...
	PoH = function()
		...
		for i = start, groupMembers do
			...
			if not subgroups[subgroup] then
				...
				member[subgroup] = NetherMachine.raid.roster[i].unit
			end
			subgroups[subgroup] = subgroups[subgroup] + min(minHeal, NetherMachine.raid.roster[i].healthMissing)
		end
		...
		if lowest then
			NetherMachine.dsl.parsedTarget = member[lowest]
			return true
		end
		return false
	end,
	...
	** DarkSim **
	...
	DarkSimUnit = function(unit)
		for index,spellName in pairs(_darkSimSpells) do
			if NetherMachine.condition["casting"](unit, spellName) then
				return true 
			end
		end
		return false
	...
	** Power word Barrier **
	...
	ClarityOfPurpose = function()
		...
		local total = 0
		local prefix = (IsInRaid() and 'raid') or 'party'
		local lowest = NetherMachine.raid.lowestHP
		for i=1,#mts.unitFriendlyCache do
			local incomingHeals = UnitGetIncomingHeals(mts.unitFriendlyCache[i].key) or 0
			local absorbs = UnitGetTotalHealAbsorbs(mts.unitFriendlyCache[i].key) or 0
			local health = mts.unitFriendlyCache[i].actualHealth + incomingHeals - absorbs
