mts = {
	Version = "0.1.4.9-GIT",
	WoW_Version = "6.1.0",
	Icon = "|TInterface\\AddOns\\NetherMachine\\NetherMachine_MrTheSoulz\\media\\logo.blp:16:16|t",
	peRecomemded = "6.1r15",
	CurrentCR = false,
...
local _parse = NetherMachine.dsl.parse
local _PeConfig = NetherMachine.config

--[[-----------------------------------------------
** Conditions **
DESC: Add condicions to NM.
---------------------------------------------------]]
NetherMachine.condition.register('twohand', function(target)
  return IsEquippedItemType("Two-Hand")
end)

NetherMachine.condition.register('onehand', function(target)
...
---------------------------------------------------]]
NetherMachine.command.register('mts', function(msg, box)
...
Replaces NM build in one beacuse NM's is over sensitive.
...
function mts.Infront(unit)
	if UnitExists(unit) and UnitIsVisible(unit) then
		...
		-- Fallback to NM's
			...
			return NetherMachine.condition["infront"](unit)
		end
	end
end
...
--[[-----------------------------------------------
** mts.Distance **
DESC: Sometimes NM's behaves badly,
So here we go...
...
function mts.Distance(a, b)
	if UnitExists(a) and UnitIsVisible(a) and UnitExists(b) and UnitIsVisible(b) then
		...
		-- Fallback to NM's
			...
			return NetherMachine.condition["distance"](b)
		end
	end
	return 0
end
