...
local function mts_updateLiveGUI()
	LiveWindow.elements.current_Queue:SetText(NetherMachine.current_spell or "\124cff0070DEWaiting...")
	LiveWindow.elements.current_spell:SetText(NetherMachine.parser.lastCast == "" and "\124cff0070DENone" or NetherMachine.parser.lastCast)
	LiveWindow.elements.current_AoE:SetText(((FireHack or oexecute) and fetch('mtsconf', 'SA') and NetherMachine.config.read('button_states', 'multitarget', false) == false and "\124cff0070DESmart AoE" or "\124cff0070DEForced AoE") or (NetherMachine.config.read('button_states', 'multitarget', false) == true and "\124cff0070DEON" or "\124cffC41F3BOFF"))
	LiveWindow.elements.current_Interrupts:SetText(NetherMachine.config.read('button_states', 'interrupt', false) == true and "\124cff0070DEON" or "\124cffC41F3BOFF")
	LiveWindow.elements.current_Cooldowns:SetText(NetherMachine.config.read('button_states', 'cooldowns', false) == true and "\124cff0070DEON" or "\124cffC41F3BOFF")
end

function mts.ShowStatus()
	if not mts_OpenLive and fetch('mtsconf','LiveGUI') then
		LiveWindow = NetherMachine.interface.buildGUI(mts_live)
