...
function Reel:SetStatus(status)
  ReelStatusText:SetText(format("… %s …", status))
end
...
function Reel:FishingFailed()
  ...
  Reel.Buffs = {
    { id = 85973, buff = 125167, condition = function() return GetCurrentMapContinent() == CONTINENT_PANDARIA end }, -- Ancient Pandaren Fishing Charm
  ...
  -- Get the skill bonus provided by a lure.
  --
  -- @returns The skill bonus.
  local function GetLureBonus(lureID)
    for i, lure in ipairs(Reel.Lures) do
      if lure.id == lureID then
        return lure.bonus
      end
    end
  ...
  -- Get the best lure (the one that provides the highest bonus).
  --
  ...
  local function GetBestLure()
    ...
    for i, lure in ipairs(Reel.__equipment.lures) do
      bonus = GetLureBonus(lure)
...
Reel:AddState("Fishing", Fishing)
  ...
  -- if not then
  --   if fishing and not fishing["_cancelled"] then
  --     fishing:Cancel()
  --     print("Stopped normal baiting")
  --   end
  --   if not ocean or ocean["_cancelled"] then
  --     local function Ocean()
  --       for i=1,20 do
  --         buffname = UnitBuff("player",i)
  --         if buffname and type(string.find(buffname,"Sea Scorpion Bait"))=="number" then
  --           return
  --         end
  --       end
  --       if GetItemCount(110292)>0 then
  --         print("Applying Sea Scorpion bait.")
  --         UseItemByName(110292)
  --       end
  --     end
  --     ocean = C_Timer.NewTicker(1, Ocean)
  --     print("Ocean baiting automatically.")
  --   else
  --     ocean:Cancel()
  --     print("Stopped ocean baiting.")
  --   end
  -- end
  -- if ocean and not ocean["_cancelled"] then
  --   ocean:Cancel()
  --   print("Stopped ocean baiting")
  -- end
