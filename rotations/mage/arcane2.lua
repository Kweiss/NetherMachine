...
-- Arcane Mage - WoD 6.0.3
-- Updated on Dec 21st 2014
...
NetherMachine.rotation.register_custom(62, "bbMage Arcane", {
	-- COMBAT ROTATION
	-- PAUSES
	...
	{ "Rune of Power", { "talent(6, 2)", "!player.buff(Rune of Power)", "!modifier.last" }, "player.ground" }, -- TODO: Track Rune distane from player
	{ "Nether Tempest", { "talent(5, 1)", "!target.debuff(Nether Tempest)", "player.debuff(Arcane Charge).count > 3" } },
	{ "Nether Tempest", { "talent(5, 1)", "target.debuff(Nether Tempest)", "player.debuff(Arcane Charge).count > 3", "target.debuff(Nether Tempest).duration < 3.6" } },
...
NetherMachine.rotation.register_custom(62, "bbMage Arcane (Alpha)", {
-- COMBAT ROTATION
	-- PAUSES
	{ "pause", "modifier.lcontrol" },
	{ "pause", "@bbLib.pauses" },
	...
	-- Cooldowns
	{ "#trinket1", "modifier.cooldowns" }, -- Trinket
	{ "#trinket2", "modifier.cooldowns" }, -- Trinket
	{ "12042", { "modifier.cooldowns", "player.spell(Evocation).cooldown <= 2", "player.mana > 50", "player.debuff(36032).count = 4" } }, -- Arcane Power (on burn phase)
	{ "Evocation", { "modifier.cooldowns", "player.mana <= 50", "player.debuff(36032).count = 4" } }, -- Evocation transition
	...
	-- Mage Bombs
	{ "114923", { "!target.debuff(114923)", "player.debuff(36032).count = 4" }, "target"}, -- Nether Tempest
	{ "114923", { "target.debuff(114923).duration <= 3", "player.debuff(36032).count = 4" }, "target"}, -- Nether Tempest Refresh
	{ "152087", { "modifier.lalt", "!player.moving", "player.spell(Evocation).cooldown <= 2", "player.debuff(36032).count = 4" }, "target.ground"}, -- Prismatic Crystal
	{ "157980", { "modifier.lalt", "!player.buff(79683).count = 3", "player.mana < 94" }, "target"}, -- SuperNova
	...
	{ "30451", { "player.moving", "player.buff(108839)" } }, -- Arcane Blast
	{ "44425", { "player.moving", "player.spell(Evocation).cooldown > 2", "player.debuff(36032).count = 4", "!player.buff(79683)" } },-- Arcane Barrage Moving

	-- Rotation
	{ "116011", { "modifier.lcontrol", "!player.moving", "!player.buff(Rune of Power)" }, "player.ground"}, -- Rune of Power
	{ "5143", { "!player.moving", "player.spell(Evocation).cooldown <= 2", "player.buff(79683).count >= 1", "player.debuff(36032).count == 4" } },-- Arcane Missiles on Burn Phase
	{ "30451", { "!player.moving", "player.spell(Evocation).cooldown <= 2" } }, -- Arcane Blast on Burn Phase
	{ "5143", { "!player.moving", "player.spell(Evocation).cooldown > 2", "player.buff(79683).count = 3", "player.debuff(36032).count == 4" } },-- Arcane Missiles 3 charges
	{ "30451", { "!player.moving", "player.spell(Evocation).cooldown > 2", "player.mana >= 93" } }, --Arcane Blast
	{ "5143", { "!player.moving", "player.spell(Evocation).cooldown > 2", "player.buff(79683).count >= 1", "player.debuff(36032).count == 4" } },-- Arcane Missiles
	{ "44425", { "!player.moving", "player.spell(Evocation).cooldown > 2", "player.debuff(36032).count == 4", "!player.buff(79683)" } },-- Arcane Barrage
	{ "30451", { "!player.moving", "player.spell(Evocation).cooldown > 2" } }, -- Arcane Blast
